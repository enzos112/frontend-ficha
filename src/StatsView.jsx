// StatsView.jsx — Estadísticas del padrón (datos quemados, Enzo conecta /api/get-estadisticas)
    ...
    <div className="sv-root">
      ...
      <main className="sv-main">
        ...
        <div className="sv-tabs">
          {TABS.map(t => (
            <button key={t} className={`sv-tab ${tab === t ? "sv-tab-on" : ""}`} onClick={() => setTab(t)}>
              {t}
            </button>
          ))}
        </div>
              ...
              {d.porPerfil.map((p, i) => (
                <div key={p.perfil} className="sv-barra-item" style={{ animationDelay: `${i * 60}ms` }}>
                  ...
                  <div className="sv-barra-track">
                    <div className="sv-barra-fill" style={{ width: `${p.pct}%`, background: p.color }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              {d.porRegion.map((r, i) => (
                <div key={r.region} className="sv-region-card" style={{ animationDelay: `${i * 50}ms` }}>
                  ...
                  <div className="sv-region-bar">
                    <div className="sv-region-bar-fill" style={{ width: `${r.pct * 2.5}%` }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="sv-donut-row">
                {d.porEdad.map(e => (
                  <div key={e.rango} className="sv-donut-item">
                    <div
                      className="sv-donut-circle"
                      style={{ background: `conic-gradient(#FF6B00 0% ${e.pct}%, #1a1a1a ${e.pct}% 100%)` }}
                    >
...
function SvStyles() {
  return (
    <style>{`
      ...
      .sv-back { background: transparent; border: none; color: #FF6B00; font-family: var(--f-bloque); font-size: 11px; cursor: pointer; white-space: nowrap; }
      ...
      .sv-main { flex: 1; padding: 16px 16px 40px; display: flex; flex-direction: column; gap: 14px; }
      ...
      .sv-hero-stripe { position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg,#FF6B00,#FFFF00,#FF00AA,#00FF41); }
      ...
      .sv-hero-num { font-family: var(--f-titulo); font-size: clamp(40px,12vw,56px); color: #FFFF00; line-height: 1; margin-bottom: 4px; }
      ...
      .sv-tab-on { border-color: #FF6B00 !important; color: #FF6B00 !important; background: rgba(255,107,0,0.08) !important; }

      /* Section */
      .sv-section { display: flex; flex-direction: column; gap: 10px; }
      .sv-section-title { font-family: var(--f-titulo); font-size: 14px; letter-spacing: 1px; color: #fff; margin-bottom: 2px; }

      /* Barras perfiles */
      .sv-barras { display: flex; flex-direction: column; gap: 10px; }
      .sv-barra-item {
        background: #111; border: 1px solid #1a1a1a; border-radius: 10px;
        padding: 10px 12px; animation: item-in 0.35s ease both;
      }
      ...
      .sv-demo-lbl   { font-family: var(--f-bloque); font-size: 10px; color: #FF6B00; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 14px; }

      .sv-donut-row  { display: flex; justify-content: center; gap: 40px; }
      .sv-donut-item { display: flex; flex-direction: column; align-items: center; gap: 8px; }
      .sv-donut-circle { width: 80px; height: 80px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
      .sv-donut-inner  { width: 56px; height: 56px; border-radius: 50%; background: #111; display: flex; align-items: center; justify-content: center; }
      ...
      .sv-genero-lista { display: flex; flex-direction: column; gap: 10px; }
      ...
      .sv-nota { font-family: var(--f-bloque); font-size: 9px; color: #2a2a2a; text-align: center; text-transform: uppercase; letter-spacing: 1px; }
    `}</style>
  );
}